...
#reviews is reviews_df in lecture 2 code
...
# Moeten we "context = document gebruiken of context = window?"
# Moeten we count = frequency of count = boolean gebruiken?
...
PCA
```{r}
review_tdm <- reviews %>% unnest_tokens(word,text) %>% count(word,ID,sort=TRUE) %>%ungroup()%>%cast_tdm(word,ID,n)
