...
text.df <- read_csv("data/OnTheSnow_SkiAreaReviews.csv")
...
j<-1 
for (j in 1:nrow(reviews)) {
 stemmed_Review<-  anti_join((reviews[j,] %>% unnest_tokens(word,Review,to_lower=TRUE)),
                             toremove)
  
 reviews[j,"Review"]<- paste((stemmed_Review[,"word"]),collapse = " ")
